...
```json title="opencode.json" "llama.cpp" {5, 6, 8, 10-24}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
- For reasoning-capable `llama.cpp` models, `options.enable_thinking: false` disables thinking by default, and a variant like `"thinking": { "enable_thinking": true }` lets `/thinking` turn it back on.
