...
Toggle the model's thinking/reasoning mode for the current session. On providers that expose reasoning controls through model variants, this switches between a thinking-enabled and thinking-disabled variant.

:::note
For local `llama.cpp`-style OpenAI-compatible models, this can be wired to `enable_thinking: true/false` through `provider.models.<model>.options` and `variants`.
:::
...
To only show or hide reasoning blocks in the UI, use:
