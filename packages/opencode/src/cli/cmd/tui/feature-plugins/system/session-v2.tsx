      ...
      <box paddingLeft={3} flexDirection="row" gap={1}>
        <box flexShrink={0}>
          <Switch>
            ...
            <Match when={complete()}>
              <text fg={props.iconColor ?? fg()} attributes={attributes()}>
                {props.icon}
              </text>
            </Match>
            <Match when={true}>
...
function ShellThread(props: ToolProps) {
  ...
  const ids = createMemo(() => {
    ...
    const single = stringValue(props.input.threadID) ?? stringValue(props.metadata.threadID)
  ...
  const exit = createMemo(() => {
    ...
    return numberValue(props.metadata.exit) ?? (props.metadata.exit === null ? null : undefined)
  })
  ...
  const timeoutMs = createMemo(() => numberValue(props.metadata.timeoutMs))
...
function shellThreadRows(metadata: Record<string, unknown>): ShellThreadRow[] {
  ...
  return metadata.threads.flatMap((item) => {
    ...
    const exit = numberValue(thread.exit) ?? (thread.exit === null ? null : undefined)
...
function shellThreadBody(raw: string): { body: string; notice?: string } {
  const timeout = raw.match(/\n*<wait_timeout[^>]*>\n([\s\S]*?)\s*$/)
  if (!timeout) return { body: raw === "(no output)" ? "" : raw }
  const body = raw.slice(0, timeout.index).trim()
  return { body: body === "(no output)" ? "" : body, notice: timeout[1]?.trim() }
}
