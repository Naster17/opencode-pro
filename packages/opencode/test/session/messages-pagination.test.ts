...
describe("MessageV2.filterCompacted", () => {
  ...
  test("starts the latest segment at the compaction boundary", async () => {
    await WithInstance.provide({
      directory: root,
      fn: async () => {
        const session = await svc.create({})
  ...
  test("fork preserves latest-segment projection after compaction", async () => {
    await WithInstance.provide({
      directory: root,
      fn: async () => {
        const session = await svc.create({})
        ...
        expect(childFiltered.map((item) => item.info.role)).toEqual(parentFiltered.map((item) => item.info.role))
        expect(
          childFiltered.some(
            (item) => item.info.role === "user" && item.parts.some((part) => part.type === "compaction"),
          ),
        ).toBe(true)
        expect(childFiltered.some((item) => item.info.role === "assistant" && item.info.summary)).toBe(true)
  ...
  test("drops earlier turn messages even when compaction starts inside a turn", async () => {
    await WithInstance.provide({
      directory: root,
      fn: async () => {
        const session = await svc.create({})
