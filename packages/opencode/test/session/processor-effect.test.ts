...
it.live("session.processor effect tests stop on structured context overflow when auto compaction is disabled", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        ...
        const chat = yield* session.create({})
        const parent = yield* user(chat.id, "compact disabled")
        const msg = yield* assistant(chat.id, parent.id, path.resolve(dir))
        const mdl = yield* provider.getModel(ref.providerID, ref.modelID)
        const handle = yield* processors.create({
          assistantMessage: msg,
        ...
        const value = yield* handle.process({
          ...
          agent: agent(),
          system: [],
          messages: [{ role: "user", content: "compact disabled" }],
          tools: {},
        })
    ...
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        if (call?.state.status === "error") {
          expect(call.state.error).toBe("Tool call interrupted before input completed")
          expect(call.state.metadata?.interrupted).toBe(true)
          expect(call.state.metadata?.interruptedRaw).toBe(JSON.stringify({ cmd: "pwd" }).slice(0, 6))
