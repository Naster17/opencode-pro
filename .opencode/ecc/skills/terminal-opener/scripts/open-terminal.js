...
function validateExecutable(value) {
  ...
  if (whitespaceIndex >= 0 && !resemblesExecutablePath) {
    throw new Error("Executable must be one argv entry, not an interpolated shell command string.")
  }
...
function detectTerminalCapability(plan, spawnSyncImpl = childProcess.spawnSync) {
  ...
  if (result.status !== 0) {
    return unavailableCapability(plan, "probe-failed", `Terminal version probe exited with status ${result.status}.`)
  }
  return {
    terminal: plan.terminal,
    supported: true,
...
function launchDetached(command, args, cwd, spawnImpl, onDetachedError) {
  ...
  if (typeof child.once === "function") {
    child.once("error", (error) => {
      onDetachedError(new Error(`Unable to start ${command}: ${error.message}`, { cause: error }))
    })
  }
  child.unref()
}
...
function launch(plan, dependencies = {}) {
  ...
  const muxFailure = muxResult.error
    ...
    : `${plan.command} cli spawn exited with status ${muxResult.status}: ${String(muxResult.stderr || "").trim()}`

  launchDetached(plan.fallback.command, plan.fallback.args, plan.cwd, spawnImpl, onDetachedError)
...
function printPlan(plan, json) {
  ...
  process.stdout.write(`Open ${plan.executable} in ${plan.terminal} using ${plan.launchMode} mode.\n`)
}
