...
function commandRoute(command, params) {
  ...
  if (command === "basket-detail" && params["basket-id"]) {
    return {
      access: "anonymous",
      pathname: `/api/baskets/${enc(params["basket-id"])}/bootstrap`,
      fixed: { stream: "1" },
      allowed: new Set(),
      consumed: ["basket-id"],
    }
  }
  if (command === "list-baskets")
    return { access: "keyed", pathname: "/baskets", allowed: new Set(["page", "per-page"]) }
  if (command === "search-markets")
    return {
      access: "keyed",
      pathname: "/markets/search",
      allowed: new Set(["platform", "category", "expiration", "limit"]),
    }
  if (command === "get-market" && params["market-id"])
    return {
      access: "keyed",
      pathname: `/markets/${enc(params["market-id"])}`,
      allowed: new Set(["platform"]),
      consumed: ["market-id"],
    }
  if (command === "market-history" && params["market-id"])
    return {
      access: "keyed",
      pathname: `/markets/${enc(params["market-id"])}/history`,
      allowed: new Set(["platform", "days"]),
      consumed: ["market-id"],
    }
  fail(
    "USAGE",
    "Use basket-index, basket-detail --basket-id ID, list-baskets, search-markets, get-market --market-id ID, or market-history --market-id ID",
    {},
    2,
  )
}
...
function safeBaseUrl(raw, envName) {
  ...
  try {
    ...
    fail("CONFIG", `${envName} must be an absolute URL`)
  }
...
function buildRequest(options, environment) {
  ...
  if (route.access === "keyed") {
    ...
    if (!apiKey)
      fail(
        "AUTH_MISSING",
        "No Itô market API credential is configured. Set ITO_API_KEY outside chat, or use the anonymous basket-index/basket-detail commands.",
      )
...
function validatePublicContract(command, body) {
  if (body?.contractVersion !== PUBLIC_CONTRACT_VERSION) {
    fail(
      "INVALID_RESPONSE",
      `Public basket read contract changed or missing (expected ${PUBLIC_CONTRACT_VERSION}); refusing to treat the response as current product data`,
    )
  }
...
async function run(options, environment = process.env, fetchImpl = fetch) {
  ...
  try {
    ...
    fail("INVALID_RESPONSE", "Itô basket API returned non-JSON content")
  }
  if (response.status === 401 || response.status === 403)
    fail("AUTH_REJECTED", "Itô rejected the credential or required read scope")
  if (response.status === 429) {
    const retry = Number(response.headers.get("retry-after"))
    fail(
      "RATE_LIMITED",
      "Itô basket API rate limit reached",
      Number.isFinite(retry) ? { retry_after_seconds: retry } : {},
    )
  }
  if (!response.ok)
    fail("UPSTREAM_ERROR", `Itô basket API returned HTTP ${response.status}`, { status: response.status })
  if (route.access === "anonymous") validatePublicContract(options.command, body)
  const rateLimit = {}
  for (const [field, header] of [
    ["limit", "x-ratelimit-limit"],
    ["remaining", "x-ratelimit-remaining"],
    ["reset_epoch", "x-ratelimit-reset"],
  ]) {
  ...
  return {
    ...
    freshness: {
      ...
      caveat:
        "Snapshot at retrieval time; verify source timestamps before acting. An edge stale marker means stale provenance even when generated_at is recent.",
    },
  ...
  Promise.resolve()
    ...
    .catch((error) => {
      ...
      process.stderr.write(
        `${options.json ? JSON.stringify(payload, null, 2) : `${payload.error.code}: ${payload.error.message}`}\n`,
      )
