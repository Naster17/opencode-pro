...
function verifyToollessSupport(dependencies = {}) {
  ...
  for (const line of featuresText.split("\n")) {
    const match = line
      .trim()
      .match(/^(\S+)\s+(stable|under development|experimental|deprecated|removed)\s+(true|false)$/)
    if (match) stages.set(match[1], match[2])
  }
  const unavailable = REQUIRED_TOOLLESS_FEATURES.filter((feature) => stages.get(feature) !== "stable")
...
function buildEnvironment(sourceEnv = process.env) {
  ...
  return Object.fromEntries(allowed.filter((name) => sourceEnv[name]).map((name) => [name, sourceEnv[name]]))
}
...
function runStdinReview(options, dependencies = {}) {
  ...
  stdin.on("end", () => {
    if (promptOverflow) {
      stderr.write(`external review absent: review packet exceeds ${MAX_PROMPT_BYTES} bytes\n`)
      setExitCode(1)
      return
    }
    try {
